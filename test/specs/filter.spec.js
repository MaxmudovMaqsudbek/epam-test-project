...
describe("Filter Functionality Spec", () => {
  ...
  it("should filter movie by User Rating", async () => {
    ...
    await filterPage.setUserRatingFilter(filterData.ratingRange.min, filterData.ratingRange.max);
    await filterPage.closeFilterMenu();
    const movies = await filterPage.moviesList;
    expect(movies.length).to.be.greaterThan(0);
    await filterPage.validateRatingRange(filterData.ratingRange.min, filterData.ratingRange.max);
  });
  it("should filter movies by Number of Votes", async () => {
    await filterPage.openTop250Movies();
    await filterPage.openFilterMenu();
    await filterPage.setUserVoteFilter(filterData.voteRange.min, filterData.voteRange.max);
    await filterPage.closeFilterMenu();
    const movies = await filterPage.moviesList;
    expect(movies.length).to.be.greaterThan(0);
    await filterPage.validateVotesAreWithinRange(filterData.voteRange.min, filterData.voteRange.max);
