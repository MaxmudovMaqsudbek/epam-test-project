...
describe("User Access Management Spec", () => {
    ...
    describe("Negative Tests with invalid credentials", () => {
            it("should show error when email is missing", async () => {
                ...
                await userAccessManagementPage.enterEmail(userAccessManagementData.emptyEmail);
                const errorText = await userAccessManagementPage.getErrorMessage(2);
                expect(errorText).to.include(userAccessManagementData.messageForEmptyEmail);
            });

            it("should show error for invalid email format", async () => {
                await userAccessManagementPage.clickSignIn();
                await userAccessManagementPage.enterEmail(userAccessManagementData.wrongEmailFormat);
                const errorText = await userAccessManagementPage.getErrorMessage(3);
                expect(errorText).to.include(userAccessManagementData.messageForWrongEmailFormat);
            });

            it("should show error when email not exists", async () => {
                await userAccessManagementPage.clickSignIn();
                await userAccessManagementPage.enterEmail(userAccessManagementData.nonExistentEmail);
                const errorText = await userAccessManagementPage.getErrorMessage(0);
                expect(errorText).to.include(userAccessManagementData.messageForNonExistentEmail);
            });
     });
    ...
    describe.skip("Positive Tests with valid credentials", () => {
        it("should login successfully with valid credentials", async () => {
            await userAccessManagementPage.clickSignIn();
            await userAccessManagementPage.enterEmail(userAccessManagementData.validEmail);
            await userAccessManagementPage.enterPassword(userAccessManagementData.password);    
